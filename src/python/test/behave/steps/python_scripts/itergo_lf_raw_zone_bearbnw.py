...
def bearbnw_wrapper_rawvault():
	...
	psid = start_job_step_execution(pid, key_obj['table_name'].lower()+'_load_rawvault_'+key_obj['tenant_name'].lower()+'_'+key_obj['application_name'],curs)
	try:
		load_rawvault(spark, key_obj, pid, psid, curs)
	except:
		print(traceback.format_exc())
		set_status_job_step_execution(psid, key_obj['table_name'].lower()+'_load_rawvault_'+key_obj['tenant_name'].lower()+'_'+key_obj['application_name'], 'error', 1, curs)
	else:
		set_status_job_step_execution(psid, key_obj['table_name'].lower()+'_load_rawvault_'+key_obj['tenant_name'].lower()+'_'+key_obj['application_name'], 'finished', 0, curs)
