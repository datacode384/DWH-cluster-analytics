...
def wrapper_script(target_directory, con):
	...
	while db2.fetch_row(res)!=False:
		...
		wrapper_type=""
		if "_SAP_" in job_name:
			if "RI" in job_name:
				table_name = job_name.partition("RI_")[2]
				wrapper_type ="rawvault"
			elif "CI" in job_name:
				table_name = job_name.partition("CI_")[2]
				wrapper_type="core"
...
def target_data_model(target_directory, db, con):
	...
	if db is "raw_zone":
		res = db2.exec_immediate(con,"SELECT TABLE_NAME_DWH, DDL from TABLE_REPO.V_DDL_RAW_VAULT WHERE TABLE_NAME_DWH like 'PAR_%'")
	else:
		res = db2.exec_immediate(con,"SELECT TABLE_NAME_DWH,DDL from TABLE_REPO.V_DDL_"+database+" WHERE TABLE_NAME_DWH like 'PAR_%'")
	while db2.fetch_row(res)!=False:
...
def load_all(target_directory, con):
	...
	source_system = ['pas_', 'bpms_', 'par_', 'dwh_']
	with open(target_directory+"/"+"load_all_sap.py","w+") as f:
		iterations = [1,2]
		tables =[]
		...
		for iteration in iterations:
			...
			for i in exceptions:
				...
				if iteration is 2:
					...
					f.write(rz_call)
					f.write(c_call)
			res = db2.exec_immediate(con,"SELECT VALUE,STRING_FLAG from JOB_REPO.PARAMETER_GROUP_VALUE where PARAMETER_GROUP_ID = 'SAP_BP'")
			while db2.fetch_row(res)!=False:
